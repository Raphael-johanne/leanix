...
const fetchGraphQLData = async () => {
  ...
  try {
    lx.showSpinner();
    averageCompletion.value = await lx
      .executeGraphQL(query, { factSheetType: selectedFactSheetType.value })
      .then(({ allFactSheets }) => {
...
const updateChart = () => {
  ...
  const backgroundColor = [
    gradient.rgbAt(averageCompletion.value).toHexString(),
    "rgba(0, 0, 0, 0.1)",
  ];
...
const factSheetTypeLabel = computed(() =>
  selectedFactSheetType.value !== null
...
<template>
  <div class="container mx-auto text-md text-gray-800">
    <!-- chart container -->
    <div
      class="relative flex flex-col flex-wrap items-center mt-16 -mx-8 mt-16"
    >
      <!-- chart title -->
